...
async function trackProductView(product) {
    ...
    const now = Date.now();
    const lastSeen = parseInt(viewedProducts[key], 10) || 0;
    if (now - lastSeen < 60 * 1000) return; // prevent duplicate rapid requests

    viewedProducts[key] = now;
    ...
    // Backend (Apps Script) expects `recordView` and `product_id`.
    // We also send the legacy parameter names to keep compatibility.
    formData.append('action', 'recordView');
    formData.append('product_id', String(product.id || '').toLowerCase());
