        ...
        for (let i = 1; i < lines.length; i++) {
            try {
                ...
                products.push({
                    ...
                    price:
                        idxPrice > -1
                            ? Number(cols[idxPrice]) || 0
                            : 0,
                    cat:
                        idxCat > -1
                            ? String(cols[idxCat] || '').trim()
                            : 'all',
                    tag:
                        idxTag > -1
                    ...
                    discount:
                        idxDiscount > -1 && cols[idxDiscount]
                            ? parseFloat(String(cols[idxDiscount]).replace(',', '.')) || 0
                            : 0,
                    img_view:
                        idxImgView > -1 && cols[idxImgView]
                            ? String(cols[idxImgView]).trim()
                            : '',
                    imgs:
                        idxImgs > -1 && cols[idxImgs]
                            ? String(cols[idxImgs])
                                .split('|')
                                .map(s => s.trim())
                                .filter(Boolean)
                    ...
                    stock:
                        idxStock > -1 && cols[idxStock]
                            ? parseInt(String(cols[idxStock]).replace(/\D/g, ''), 10) || 0
                            : 0,
...
async function fetchCSVFromSources(sources, timeoutMs = 10000) {
    ...
    const proxySources = sources.filter(source => /allorigins|r\.jina\.ai/i.test(String(source)));
    const orderedSources = isFileOrigin && proxySources.length ? proxySources : sources;
    let lastError = null;
...
async function fetchProducts() {
    ...
    try {
        ...
        const ratingsText = await fetchCSVFromSources([
            ...
            "https://r.jina.ai/http://docs.google.com/spreadsheets/d/e/2PACX-1vTAa67zBZTQExpINm7B5qe0YGU5NhAEG0TaNiLwK1MT9TrYeZR3ofzonbMO7rGAEYCg4vtBwATKzRaL/pub?gid=1862185571&single=true&output=csv"
        ]);
        ratingsData = parseRatingsCSV(ratingsText);
    ...
    for (let i = 0; i < PRODUCT_CSV_SOURCES.length; i++) {
        ...
        try {
            ...
            const csvText = await fetchCSVFromSources([
                ...
                "https://r.jina.ai/http://" + source.replace(/^https?:\/\//, '')
            ]);
...
async function fetchImages() {
    const sources = [
        ...
        "https://r.jina.ai/http://docs.google.com/spreadsheets/d/e/2PACX-1vTAa67zBZTQExpINm7B5qe0YGU5NhAEG0TaNiLwK1MT9TrYeZR3ofzonbMO7rGAEYCg4vtBwATKzRaL/pub?gid=1315225138&single=true&output=csv"
    ];
    let lastError = null;
    try {
